...
window that displays an animated pet and wanders around the screen.

States: idle -> jump (moving) -> sleep -> happy (on click), looping.
"""
...
class DesktopPet:
    ...
    def _animate(self):
        frames = self.sprites.frames[self.state]
        self.label.config(image=frames[self.frame_index % len(frames)])
        self.frame_index += 1

        fps = self.sprites.fps[self.state]
        delay_ms = max(1, int(1000 / fps))
        self.root.after(delay_ms, self._animate)

    def _tick_behavior(self):
        """Runs once a second; decides on state transitions."""
        if not self.dragging:
            self.idle_timer += 1

            if self.state == "idle" and self.idle_timer > self._next_idle_threshold:
                if random.random() < 0.65:
                    self._start_hop()
                else:
                    self._set_state("sleep")
            elif self.state == "sleep" and self.idle_timer > self._next_sleep_threshold:
                self._set_state("idle")
